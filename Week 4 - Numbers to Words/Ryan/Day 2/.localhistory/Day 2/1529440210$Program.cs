...
namespace Day_2
{
    internal class Program
    {
        ...
        internal static Dictionary<string, long> _multipliers = new Dictionary<string, long>
        {
            {"dollars", 1 },
            {"thousand", 1000},
            { "million", 1000000},
        ...
        public static string NumbersToWords(string number)
        {
            ...
            string reversedNumber = String.Join("", number.Reverse());
            var chunks = new List<string>();

            for (var i = 0; i < reversedNumber.Length; i++)
            {
                var c = reversedNumber[i];
                if (i % 3 == 0)
                {
            ...
            for (var i = 0; i < chunks.Count; i++)
            {
                ...
                {
                    words += setPrefix ? " " + GetPrefix(chunks.Count - 1 - i) : "";
                    continue;
                }
                else if (chunk.Length > 1 && _intToWord.ContainsKey(chunk[1].ToString() + chunk[0]))
                {
                ...
                {
                    words += " " + _intToWord[chunk[0].ToString()];
                    if (chunk[0] == '0' && i == chunks.Count-1 && words.Trim() == "")
                        words += "zero";
                }
        ...
        public static long WordsToNumbers(string numberText)
        {

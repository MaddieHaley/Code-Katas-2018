    ...
    internal class Program
    {
        ...
        public static string GetPrefix(int chunk)
        {
            if (chunk < Multipliers.Count())
                return Multipliers.Reverse[(long) Math.Pow(1000, chunk)];
        ...
        public static string NumbersToWords(long number)
        {
            ...
            for (var count = 0; number != 0; count++)
            {
                ...
                if (hundreds != 0)
                {
                    wordChunk += WordToInt.Reverse[hundreds] + " hundred";
                    emptyChunk = false;
                }

                if (tens != 0)
                {
                    wordChunk += (!emptyChunk ? " " : "") + WordToInt.Reverse[tens];
                    emptyChunk = false;
                }

                if (!emptyChunk && count>0) wordChunk += " " + GetPrefix(count);

                words = wordChunk + (words.Length > 0 && words[words.Length-1] != ' ' ? " " + words : "");

...
namespace Day_4
{
    internal class Program
    {
        ...
        {
            if (chunk < Multipliers.Forward.Count)
                return Multipliers[chunk];

            return GetPrefix(chunk - Multipliers.Count) + Multipliers[Multipliers.Count - 1];
        }
        ...
        public static string NumbersToWords(long number)
        {
            ...
            for (int count = 0; number != 0; count++)
            {
                ...
                {
                    emptyChunk = false;
                    words += " " + WordToInt.Reverse[hundreds];
                }

                if (tens != 0)
                {
                    emptyChunk = false;
                    words += " " + WordToInt.Reverse[tens];
                }

                if (!emptyChunk)
                {
                    words += " " + Multipliers.Reverse[count];
                }

    ...
    internal class Program
    {
        ...
        public static string NumbersToWords(long number)
        {
            ...
            for (var count = 0; number != 0; count++)
            {
                ...
                {
                    emptyChunk = false;
                    words = WordToInt.Reverse[hundreds] + (words.Length > 0 ? " " + words : "");
                }

                if (tens != 0)
                {
                    emptyChunk = false;
                    words = WordToInt.Reverse[tens] + (words.Length > 0 ? " " + words : "");
                }

                if (!emptyChunk) words = GetPrefix(count) + (words.Length > 0 ? " " + words : "");
